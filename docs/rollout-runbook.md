...
## 2. Application containers — currently blocked

9. **[BLOCKED]** Build and deploy the actual `ceiba-control-plane` and `ceiba-runtime` container images. **Neither app repo currently has a Dockerfile** (confirmed absent during this pass's grounding step) — only `ceiba-runtime/docker-compose.local.yml`, which runs Postgres/Redis for local dev, not the apps themselves. A working Dockerfile needs to live inside each app repo (to `COPY` its own source and run its own build), which is outside this pass's mandate (no application-repo changes). **Recommended follow-up:** a small Builder slice in each of `ceiba-runtime` and `ceiba-control-plane` adding a production Dockerfile (multi-stage, arm64-compatible per `ADR-0002-graviton-instances.md`) before this step can be automated or even manually completed. Flagged here rather than worked around.
